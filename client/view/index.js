...
async function getDomain() {
    try {
        ...
        if (response.status !== 200) throw "não foi possível carregar a página"
...
class Html {
    ...
    renderPatient() {
        ...
        var content = "<h3>Paciente</h3>"
        content += "<table><tbody>"
        for (let key in patient) {
            const object = patient[key]
            content += `<tr><td>${object.name}</td>`
            if (key === "gender") {
                content += (
                    `<td><input id="2" type="radio" class="setGender" name="${key}"> Feminino</td>
                    <td><input id="1" type="radio" class="setGender" name="${key}"> Masculino</td>`
                ) 
            }
            else {
                content += `<td><input id="${key}" class="setPatientValue" type="text"></td>`
            }
        }
        content += "</tr></tbody></table>"
        patientElement.innerHTML = content
        Array.from(document.getElementsByClassName("setGender")).forEach(element => {
            element.addEventListener("click", event => this.setGender(event, patient))
        })
        Array.from(document.getElementsByClassName("setPatientValue")).forEach(element => {
            element.addEventListener("change", event => this.setValue(event, patient))
        })
    }

    renderFA() {
        const faElement = document.getElementById("fa")
        var content = "<h3>FA</h3>"
        content += "<table><tbody>"
        for (let key in fa) {
            let object = fa[key]
            content += (
                `<tr>
                    <td><input id="${key}" class="setFa" type="checkbox"></td>
                    <td>${object.name}</td>`
            )
            if (!object.value) {
                content += `<td><input id="${key}Text" class="setFaValue" type="text"></td>`
            }
            else {
                content += `<td>${object.value}</td>`
            }
            content += "</tr>"
        }
        content += "</tbody></table>"
        faElement.innerHTML = content
        Array.from(document.getElementsByClassName("setFa")).forEach(element => {
            element.addEventListener("click", event => this.setFactor(event, fa))
        })
        Array.from(document.getElementsByClassName("setFaValue")).forEach(element => {
            element.addEventListener("change", event => this.incluirValor(event, fa))
        })
    }

    renderFI() {
        const fiElement = document.getElementById("fi")
        var content = "<h3>FI</h3>"
        content += "<table><tbody>"
        for (let key in fi) {
            let object = fi[key]
            content += (
                `<tr>
                    <td><input id="${key}" class="setFi" type="checkbox"></td>
                    <td>${object.name}</td>`
            )
            if (!object.value) {
                content += `<td><input id="${key}Text" class="setFiValue" type="text"></td>`
            }
            else {
                content += `<td>${object.value}</td>`
            }
        ...
        content += "</tbody></table>"
        fiElement.innerHTML = content
        Array.from(document.getElementsByClassName("setFi")).forEach(element => {
            element.addEventListener("click", event => this.setFactor(event, fi))
        })
        Array.from(document.getElementsByClassName("setFiValue")).forEach(element => {
            element.addEventListener("change", event => this.setValue(event, fi))
        })
    }

    renderFT() {
        const ftElement = document.getElementById("ft")
        var content = "<h3>FT</h3>"
        content += "<table><tbody>"
        for (let key in ft) {
            let object = ft[key]
            content += (
                `<tr>
                    <td><input id="${key}" class="setFt" type="checkbox"></td>
                    <td>${object.name}</td>`
            )
            if (!object.value) {
                content += `<td><input id="${key}Text" class="setFtValue" type="text"></td>`
            }
            else {
                content += `<td>${object.value}</td>`
            }
        ...
        content += "</tbody></table>"
        ftElement.innerHTML = content
        Array.from(document.getElementsByClassName("setFt")).forEach(element => {
            element.addEventListener("click", event => this.setFactor(event, ft))
        })
        Array.from(document.getElementsByClassName("setFtValue")).forEach(element => {
            element.addEventListener("change", event => this.setValue(event, ft))
        })


    }

    setGender(event, patient) {
